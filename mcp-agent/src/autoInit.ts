 ...
 */
export function buildPolicy(workspaceRoot: string, hours = 24 * 365): Omit<PolicyBundle, "signature"> {
  const now = Date.now();
 ...
 */
export async function loadOrCreateIdentity(filePath: string): Promise<{ idm: IdManager; created: boolean }> {
 ...
 */
export async function signAndPersistPolicy(
  workspaceRoot: string,
  hours?: number,
): Promise<{ signedPolicy: PolicyBundle; authorityIdm: IdManager }> {
 ...
 */
export async function autoInit(workspaceRoot: string): Promise<{
  serverIdm: IdManager;
  ...
  if (fs.existsSync(policyPath)) {
    ...
    signedPolicy = raw as PolicyBundle;
