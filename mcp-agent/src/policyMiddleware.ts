 ...
 */
export const DEFAULT_TOOL_MAPPINGS: Record<string, CapabilityMapping> = {
  read_file: {
    capability: "fs.read",
 ...
 */
function resolveWorkspacePath(raw: string, workspaceRoot?: string): string {
  if (!raw) return workspaceRoot ?? "/workspace";
...
export function createPolicyMiddleware(options: PolicyMiddlewareOptions) {
  ...
  async function enforce(
    ...
    const intent: ExecutionIntent = await em.createIntent({
      tool: toolName,
      argv,
      cwd: workspaceRoot,
