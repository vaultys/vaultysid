...
async function main() {
  ...
  const { idm: authorityIdm, created } = await loadOrCreateIdentity(
    configPath("authority.secret.json"),
  );
  if (created) {
  ...
  fs.writeFileSync(
    configPath("authority.identity.json"),
    JSON.stringify(
      {
        did: vid.did,
  ...
  let policyTemplate: Record<string, unknown>;
  ...
  if (policyTemplate.scopes && typeof policyTemplate.scopes === "object") {
    const scopes = policyTemplate.scopes as Record<string, string[]>;
    for (const [key, patterns] of Object.entries(scopes)) {
      if (Array.isArray(patterns)) {
        scopes[key] = patterns.map((p: string) =>
  ...
  } as Omit<PolicyBundle, "signature">;
