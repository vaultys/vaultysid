...
const instanciateContact = (c) => {
    ...
    if (c.type === 3) {
        ...
        if (c.keyManager.signer.publicKey.length === 1952) {
            ...
            //console.log(c);
            vaultysId = new VaultysId_1.default(DeprecatedKeyManager_1.default.instantiate(c.keyManager), c.certificate, c.type);
        }
        else {
...
class IdManager {
    ...
    async verifyWebOfTrust() {
        ...
        for (const certid of this.store.substore("wot").list()) {
            ...
            verified = verified && (await Challenger_1.default.verifyCertificate(buffer_1.Buffer.from(cert)));
        ...
        for (const contact of this.contacts) {
            verified = verified && (await Challenger_1.default.verifyCertificate(buffer_1.Buffer.from(contact.certificate)));
        }
        //console.log("contacts", verified);
        for (const app of this.apps) {
            verified = verified && (await Challenger_1.default.verifyCertificate(buffer_1.Buffer.from(app.certificate)));
        }
    ...
    async requestConnect(channel, contactdid) {
        let contact = this.getContact(contactdid);
        if (!contact)
            contact = this.getApp(contactdid);
        if (!contact)
            throw new Error(contactdid + " is not in the Web of Trust");
        const rand = __1.crypto.randomBytes(16);
        const dh = await this.vaultysId.performDiffieHellman(contact);
        if (!dh)
            throw new Error("Unable to perform DH");
        channel.send(rand);
        const result2 = await channel.receive();
        const result1 = (0, crypto_1.hash)("sha256", tweetnacl_1.default.scalarMult(rand, dh));
        if (result1.toString("base64") === result2.toString("base64"))
            return result1;
        else
            throw new Error("Opening session failed with contact " + contactdid);
    }
    async acceptConnect(channel, contactdid) {
        let contact = this.getContact(contactdid);
        if (!contact)
            contact = this.getApp(contactdid);
        if (!contact)
        ...
        const rand2 = buffer_1.Buffer.concat([rand.slice(0, 16), __1.crypto.randomBytes(16)]);
        const result1 = (0, crypto_1.hash)("sha256", tweetnacl_1.default.scalarMult(rand2, dh));
    }
    ...
    async requestPRF(channel, appid, accept) {
        if (appid.length < 3) {
