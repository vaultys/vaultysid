...
export const createRandomVaultysId = async (): Promise<VaultysId> => {
  ...
  switch (type) {
    ...
    case 3:
      const attestation1 = await navigator.credentials.create(SoftCredentials.createRequest(Math.random() < 0.5 ? -8 : -7, false));
      // @ts-expect-error mockup
      return VaultysId.fido2FromAttestation(attestation1);
    case 4:
      const attestation2 = await navigator.credentials.create(SoftCredentials.createRequest(Math.random() < 0.5 ? -8 : -7, true));
      // @ts-expect-error mockup
      return VaultysId.fido2FromAttestation(attestation2);
    default:
      return VaultysId.generatePerson();
...
export const allVaultysIdType = async (): Promise<VaultysId[]> => {
  ...
  if (typeof window === "undefined") {
    // let attestation = await navigator.credentials.create(SoftCredentials.createRequest(PQ_COSE_ALG.DILITHIUM5));
    // result.push(await VaultysId.fido2FromAttestation(attestation));
    let attestation = await navigator.credentials.create(SoftCredentials.createRequest(-7));
    // @ts-expect-error mockup
    result.push(await VaultysId.fido2FromAttestation(attestation));
