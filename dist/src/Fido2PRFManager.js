...
const getSignerFromCkey = (ckey) => {
    ...
    if (k.get(3) == -7)
        publicKey = Buffer.concat([Buffer.from("04", "hex"), k.get(-2), k.get(-3)]);
    else if (k.get(3) == -8)
        publicKey = k.get(-2);
    return { publicKey };
};
class Fido2PRFManager extends Fido2Manager_1.default {
    constructor() {
        super();
        this.prfsalt = Buffer.from("VaultysID salt");
    }
    ...
    getSecret() {
        return Buffer.from((0, msgpack_1.encode)({
            v: this.version,
            f: this.fid,
            t: this._transports,
            c: this.ckey,
    ...
    async getCypher() {
        if (!this.cypher?.secretKey) {
            ...
            const { 
            // @ts-expect-error prf not yet in dom
            prf: { results: { first }, }, } = result.getClientExtensionResults();
            const cypher = tweetnacl_1.default.box.keyPair.fromSecretKey(Buffer.from(first));
            this.cypher = {
