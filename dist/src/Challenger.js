...
const STEP1 = 1;
const COMPLETE = 2;
const writeString = (name, value) => Buffer.concat([Buffer.from([0xa0 + name.length]), Buffer.from(name, "ascii"), Buffer.from([0xa0 + value.length]), Buffer.from(value, "ascii")]);
const writeBuffer = (name, value) => Buffer.concat([Buffer.from([0xa0 + name.length]), Buffer.from(name, "ascii"), Buffer.from([0xc5, value.length >> 8, value.length]), value]);
...
const serializeUnsigned = (challenge, version = 0) => {
    ...
    return version === 0 ? encode_v0(picked) : Buffer.from((0, msgpack_1.encode)(picked));
};
...
class Challenger {
    ...
    async setChallenge(challengeString) {
        ...
        if (this.challenge.state === ERROR) {
            ...
            this.challenge.nonce = Buffer.concat([this.challenge.nonce || new Uint8Array(), (0, crypto_1.randomBytes)(16)]);
    ...
    async update(challengeString, metadata = {}) {
        if (this.state === ERROR) {
            ...
            if (this.state === UNINITIALISED && tempchallenge.state === INIT) {
                ...
                this.mykey = this.challenge.pk2 = this.vaultysId.id;
                this.hisKey = this.challenge.pk1;
                this.challenge.nonce = Buffer.concat([this.challenge.nonce, (0, crypto_1.randomBytes)(16)]);
                const serialized = this.getUnsignedChallenge();
