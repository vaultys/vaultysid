...
const instanciateApp = (a) => {
    return VaultysId_1.default.fromId(Buffer.from(a.serverId, "base64"), a.certificate);
};
class IdManager {
    ...
    async signIn() {
        ...
        const extractedChallenge = SoftCredentials_1.default.extractChallenge(Buffer.from(response.clientDataJSON));
    ...
    async requestSignFile(channel, file) {
        ...
        if (challenger.isComplete()) {
            channel.send(Buffer.from((0, msgpack_1.encode)(file)));
    ...
    async acceptSignFile(channel, accept) {
        ...
        if (challenger.isComplete()) {
            ...
            if (!accept || (await accept(challenger.getContactId(), file))) {
                ...
                channel.send(Buffer.from((0, msgpack_1.encode)(result)));
            }
        }
        else
            channel.send(Buffer.from([0]));
