...
const createRequest = (alg, onlyStrings = false) => {
  ...
  return {
    publicKey: {
      ...
      user: {
        id: "ertyu45678RUTYESEZTYSDYJTUE4576232453",
...
describe("SoftCredentials", () => {
  ...
  it("get assertion and verify with attestation using ECDSA", async () => {
    ...
    assert.equal(
      SoftCredentials.extractChallenge(assertion.response.clientDataJSON),
      payload.publicKey.challenge.toString("base64"),
    );
    const verified = await SoftCredentials.verify(attestation, assertion);
    assert.ok(verified);
  });
  ...
  it("verify assertion with attestation using github webauthn json coming from yubikey credentials", async () => {
    const attestationSafe = {
      ...
      rawId:
        "vLjE6ObBfDZCZSJEqt1HhDokuO6O5Fztx7DSfQts7QEaj0Cbjr0Sry-jVuKdDA3OJwzk8jhygjm6-6WskhsWMyBDcMMtitMStwY7is8G7pCKuUirDZ3-AKZYAqmnF6CqIiLBxSxvDl11iVOMcBgkFwOLf8rGqv05c3k6r2XZ0SA",
      response: {
        clientDataJSON:
          "eyJ0eXBlIjoid2ViYXV0aG4uY3JlYXRlIiwiY2hhbGxlbmdlIjoiZkl3bFU5NHNNcUsyMVNrX1BYbFIzU2hLU0JVc0t6dFVpX0pqNE5YampSOCIsIm9yaWdpbiI6Imh0dHBzOi8vdmF1bHR5cy5sb2NhLmx0IiwiY3Jvc3NPcmlnaW4iOmZhbHNlfQ",
        attestationObject:
          "o2NmbXRkbm9uZWdhdHRTdG10oGhhdXRoRGF0YVjh3k-dKamFhxtblpCasfhfE1MJJYpzErYjq2DqvChGolBBAAAAAQAAAAAAAAAAAAAAAAAAAAAAgLy4xOjmwXw2QmUiRKrdR4Q6JLjujuRc7cew0n0LbO0BGo9Am469Eq8vo1binQwNzicM5PI4coI5uvulrJIbFjMgQ3DDLYrTErcGO4rPBu6QirlIqw2d_gCmWAKppxegqiIiwcUsbw5ddYlTjHAYJBcDi3_Kxqr9OXN5Oq9l2dEgpAEBAycgBiFYIO6J63OnkjzyQ--iXSbgm1oLSl3wzlDCy4ohwkbN-l4I",
    ...
    const assertion = {
      ...
      response: {
        clientDataJSON:
          "",
        authenticatorData: "3k-dKamFhxtblpCasfhfE1MJJYpzErYjq2DqvChGolABAAAAIQ",
        signature:
          "_eq8EwX_twQyWjyiCvCDu5JLKEWpMxW9Pz0Qwpt8A5UcHJyGoGQMrssaws-jwqIpKxkVU7MBlHzHUgFULFn5Cw",
        userHandle: null,
      },
      clientExtensionResults: {},
    };
    assert.equal(
      SoftCredentials.extractChallenge(attestationSafe.response.clientDataJSON),
      "fIwlU94sMqK21Sk/PXlR3ShKSBUsKztUi/Jj4NXjjR8=",
    );
    assert.equal(
      SoftCredentials.extractChallenge(assertion.response.clientDataJSON),
      "AOmfgdnuxr/2ul8OZLzAWRMRbGt5y3lr11DqrqwQGLU=",
    );
    const verified = await SoftCredentials.verifySafe(
      attestationSafe,
      assertion,
    );
    assert.ok(verified);
  ...
  it("attestation response from yubikey should be verified", async () => {
    ...
    assert.equal(info.issuer, 'CN=Yubico FIDO Preview CA');
    assert.equal(info.subject.CN, 'Yubico U2F EE Serial 1213939126');
    assert.equal(info.version, 'v3 (2)');
    assert.ok(!info.basicConstraintsCA);
  ...
  it("no attestation in response should be valid and not be verified", async () => {
    const response =  {
      attestationObject: "",
      clientDataJSON: "eyJ0eXBlIjoid2ViYXV0aG4uY3JlYXRlIiwiY2hhbGxlbmdlIjoiWXpTYkpBbGIya0N6Qm1xa1U3cG9Nem92LXNONU5HcHRuWUFrUElkRWt4VSIsIm9yaWdpbiI6Imh0dHBzOi8vdmF1bHR5c2lkMi5sb2NhLmx0IiwiY3Jvc3NPcmlnaW4iOmZhbHNlfQ=="
    }
    assert.ok(!await SoftCredentials.verifyPackedAttestation(response));
  });
  ...
  it("verify assertion with attestation using github webauthn json coming from macosx credentials", async () => {
    const attestationSafe = {
      ...
      rawId:
        "AdGTqisA0_nFlbOQjjRCElrKVd9jMC7YOM1Lm49AoQFLXfVK8TQZovhN8YEZJaTHU7HBiYYAnCH4wIDADceW0nY_USlc_-KLMwyB_NRLyLgbQ34SULTSYjHF4tPZWO71WgwaXCxp4TPi6L2So0L3w6B_xv9WbvfCKDAp56Jk44k_lQsP431W_g5hY1Psd1r_VOIuuvP-YfluxwWS6C4mn2BLvp_Lbddhn9VU_X6w1HzHJiOZbm1Dlk0Yww",
      response: {
        clientDataJSON:
          "",
        attestationObject:
          "o2NmbXRkbm9uZWdhdHRTdG10oGhhdXRoRGF0YVkBP95PnSmphYcbW5aQmrH4XxNTCSWKcxK2I6tg6rwoRqJQRWH4It6tzgACNbzGCmSLCyXx8FUDALsB0ZOqKwDT-cWVs5CONEISWspV32MwLtg4zUubj0ChAUtd9UrxNBmi-E3xgRklpMdTscGJhgCcIfjAgMANx5bSdj9RKVz_4oszDIH81EvIuBtDfhJQtNJiMcXi09lY7vVaDBpcLGnhM-LovZKjQvfDoH_G_1Zu98IoMCnnomTjiT-VCw_jfVb-DmFjU-x3Wv9U4i668_5h-W7HBZLoLiafYEu-n8tt12Gf1VT9frDUfMcmI5lubUOWTRjDpQECAyYgASFYIM3N0z1jSTixc7fNwxy1BtBUrNUOiPOm2j7Zlwx6s8eXIlggKB-zZiuYTAaAdPVQuN5EUQeRNX0kjS2oikGCJsymvH8",
        transports: ["internal"],
      },
      clientExtensionResults: {},
    };
    const assertion = {
      ...
      response: {
        clientDataJSON:
          "eyJ0eXBlIjoid2ViYXV0aG4uZ2V0IiwiY2hhbGxlbmdlIjoibUlySjNsVWxsNXRadjhfa25kMEJEeU12VlVFcnhBY19CWWNkX2hOQ2lZZyIsIm9yaWdpbiI6Imh0dHBzOi8vdmF1bHR5cy5sb2NhLmx0IiwiY3Jvc3NPcmlnaW4iOmZhbHNlfQ",
        authenticatorData: "3k-dKamFhxtblpCasfhfE1MJJYpzErYjq2DqvChGolAFYgu7dQ",
        signature:
          "MEQCIFyGZIbrLunau93lnx0vTkWQjr_CcTkjYhyZ1-szXRPeAiBXwyQRQhMn59eva_CRlZOTQ_TLAgxr3eCWbwa1dQBnTA",
        userHandle:
          "ZGlkOnZhdWx0eXM6MzI2ZjUyY2VkNjRhYmE4YzQ4MWRkMDU3ZDFkODA0MGY0ZWE3MzZiMA",
      },
      clientExtensionResults: {},
    };
    assert.equal(
      SoftCredentials.extractChallenge(attestationSafe.response.clientDataJSON),
      "aRE1egZ6G1+AEEOIzPmlNrAgOSyEeVDn89KCDE1DdOE=",
    );
    assert.equal(
      SoftCredentials.extractChallenge(assertion.response.clientDataJSON),
      "mIrJ3lUll5tZv8/knd0BDyMvVUErxAc/BYcd/hNCiYg=",
    );
    const verified = await SoftCredentials.verifySafe(
      attestationSafe,
      assertion,
