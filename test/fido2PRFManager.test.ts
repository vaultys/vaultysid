...
describe("Fido2PRFManager", () => {
  ...
  it("should perform Diffie-Hellman key exchange between VaultysIds with Fido2Manager", async () => {
    ...
    //console.log(alice.keyManager, bob.keyManager);
  ...
  it("should work with different FIDO2 algorithm combinations", async () => {
    ...
    const attestationECDSA1 = await navigator.credentials.create(SoftCredentials.createRequest(-7, true));
    const attestationECDSA2 = await navigator.credentials.create(SoftCredentials.createRequest(-7, false));
    const attestationEdDSA = await navigator.credentials.create(SoftCredentials.createRequest(-8, true));
